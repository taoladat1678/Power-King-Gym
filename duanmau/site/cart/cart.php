                ...
                <tbody class="text-center" id="giohang">
                    ...
                    <?php foreach ($_SESSION['cart'] as $index => $item) : ?>
                        <tr>
                            <td><?= $index ?></td>
                            <td><?= $item['ten_hh'] ?></td>
                            <td><span><?= number_format($item['don_gia'], 0, ".") ?></span> đ <input type="hidden" class="don_gia_an"
                                    name="don_gia" value="<?= $item['don_gia'] ?>"></td>
                            ...
                            <td class="pt-1 m-auto">
                                <form action="delete-cart.php?act=update_sl" method="post">
                                    <input type="number" class="form-control sl" name="update_sl" onchange="this.form.submit()"
                                        value="<?= $item['sl'] ?>" min="1" max="99999">
                                    <input type="hidden" class="form-control" name="ma_hh" value="<?= $index ?>">
                                </form>
                            </td>
                            <td><span class="thanh_tien_sp"><?= number_format($item['don_gia'] * $item['sl'], 0, ".") ?></span> đ
                            </td>
                            <td class="pt-1 m-auto">
                        ...
                        <?php $totalAll += $item['don_gia'] * $item['sl']; ?>
                    <?php endforeach ?>
                </tbody>
                <tfoot id="tongdonhang">
                    <tr class="text-center">
                        <th colspan="5">Tổng thành tiền: </th>
                        <td class="text-danger font-weight-bold"><?= number_format($totalAll, 0, ".") ?> đ</td>
                        <td></td>
                    </tr>
                    <tr class="text-right">
                        <th colspan="7">
                            ...
                            <a onclick="return confirm('Bạn chắc chắn muốn xóa tất cả k??');"
                                href="<?= $SITE_URL . "/cart/delete-cart.php?act=deleteAll" ?>" class="btn btn-danger">Xóa tất
                                cả</a>
                        </th>
                    </tr>
                </tfoot>
