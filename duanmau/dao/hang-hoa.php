...
{
    $sql = "SELECT * FROM san_pham ORDER BY ma_hh desc";
    return pdo_query($sql);
}
function hang_hoa_select_by_id($ma_hh)
{
    $sql = "SELECT * FROM san_pham WHERE ma_hh=?";
    return pdo_query_one($sql, $ma_hh);
}
function hang_hoa_exist($ma_hh)
{
    $sql = "SELECT count(*) FROM san_pham WHERE ma_hh=?";
    return pdo_query_value($sql, $ma_hh) > 0;
}
function hang_hoa_exist_add($ten_hh)
{
    $sql = "SELECT count(*) FROM san_pham WHERE ten_hh=?";
    return pdo_query_value($sql, $ten_hh) > 0;
}
function hang_hoa_exist_update($ma_hh, $ten_hh)
{
    $sql = "SELECT count(*) FROM san_pham WHERE ma_hh!=? and ten_hh=?";
